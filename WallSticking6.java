...
public class WallSticking6 {
   ...
   // 0 for unexplored; 1 for visited; 2 for empty space but not visited ; 3 for obstacle; 5 for goal
     ...
     public  WallSticking6(int[] strP,double coverage_limit,int[][] occupancy,long time_limit, int speed){
       ...
       this.path2=new ArrayList(); 
        ...
        direc.add(new int[]{-1,0});
        direc.add(new int[]{0,-1});  //S
        direc.add(new int[]{1,0});   //W
        direc.add(new int[]{0,1});   //N
                                     //E
...
//             }
               ...
               if(curPos[0]==18&&curPos[1]==13){
                found_goal++; 
                //if(count>=300*coverage_limit&&found_start!=0) break;
   ...
   public void dfs(int[] pos){
        ...
        //SWNE       
            for(int i=0;i<4;i++){
                ...
                if((pos[1]<=1&&i==1)||(pos[0]>=18&&i==2)||(pos[1]>=13&&i==3)||(pos[0]<=1&&i==0)) continue;   
